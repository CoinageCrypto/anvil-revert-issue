...
contract InfinexERC2771Forwarder is EIP712 {
     ...
     */
    error InfinexERC2771ForwarderUntrustfulTarget(address target, address forwarder);

    /**
     * @dev A call to an address target failed. The target may have reverted.
     */
    error InfinexERC2771ForwarderFailedInnerCall();
    ...
    function execute(ForwardRequestData calldata request) public payable virtual {
        ...
        if (!_execute(request, true)) {
            revert InfinexERC2771ForwarderFailedInnerCall();
        }
    }
    ...
    function _validate(
        ...
        return (
            ...
            nonces[recovered][request.nonce] == false,
    ...
    function _execute(
        ...
        (bool isTrustedForwarder, bool newNonce, bool signerMatch, address signer) = _validate(request);

        // Need to explicitly specify if a revert is required since non-reverting is default for
        // batches and reversion is opt-in since it could be useful in some scenarios
        if (requireValidRequest) {
            if (!isTrustedForwarder) {
                revert InfinexERC2771ForwarderUntrustfulTarget(request.to, address(this));
            }
        ...
        if (isTrustedForwarder && newNonce && signerMatch) {
